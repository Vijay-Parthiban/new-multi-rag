...
**Web-Scrapper API** (port `8000`; the ingestion scraper path and `scrape_embeddings` depend on it). It needs a `crawler` database, which `crawler-db-migrate` creates the tables for:
...
| **Frontend** | Ingestion nav exposes Overview / Folders / Sources / Knowledge Store, with pages mounted persistently in `AppLayout.tsx` (the Upload item was removed 2026-09-20). Retrieval nav exposes 10 items (Overview, Knowledge Store, Pipelines, Chat, Prompts, Real Time Monitoring, Offline Evaluation, Guard Config, Guard Traces, Guard Evaluation). The Tracking item was removed on 2026-09-23; the retrieval Knowledge Store adds a 12th route, the read-only detail page `/knowledge-store/:id`, resolved from `useRouteParams` in `AppLayout.tsx` alongside `source-detail` |
