...
6. `execute_universal_fanout_sync(db, product)`:
   ...
   - `_chunk_pages()` splits every page with the product's chunking, so the list that reaches the writers holds one `FilePage` per chunk. `page_index` is kept, and `chunk_index` is numbered from `0` inside each page. `_assign_image_chunk_indexes()` offsets each figure past the text chunks of its own page, so two records never share a Redis key. A blank page yields no chunk;
...
4. **Chunking** — `_chunk_pages()` splits every page with `chunk_size` and `chunk_overlap`. Each figure page then gets a `chunk_index` above the text chunks of its own page.
...
- Tests: `backend/tests/test_page_yielder.py`, `test_page_yielder_figures.py`, `test_fanout_payload.py`, `test_knowledge_destination_schemas.py`, `test_knowledge_product_files.py`, `test_connector_config_validation.py`, `test_ingestion_profile_chunking.py` and `test_ingestion_modality_resolvers.py`. Chunking covers `_resolve_chunking` with no profile, with a profile, and with an overlap above the size. It also covers `_chunk_pages` over one long page, a blank page and three pages. Run with `uv run pytest tests -q` from `rag-ingestion-manager/backend`. 23 cases preceded this feature.
