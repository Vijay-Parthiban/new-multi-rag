...
DEFAULT_PROMPTS: dict[str, str] = {
    "rag_retrieval_query": "Reformulate query for hybrid retrieval: {query}",
    "rag_context_rerank": "Score context relevance for query: {query}",
}
...
def load_packaged_prompt(name: str) -> str:
    ...
    return f"RAG prompt template '{name}' query:\n{{query}}"


def load_prompt(name: str) -> str:
    override = read_override("rag_core", name)
