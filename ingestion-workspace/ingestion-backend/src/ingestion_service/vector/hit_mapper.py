...
from src.ingestion_service.types import FILE_INGEST_SOURCE_TYPE, WEB_SCRAPE_SOURCE_TYPE
...
def map_scored_point(hit: qmodels.ScoredPoint) -> dict[str, object]:
    ...
    source_type = str(payload.get("source_type") or FILE_INGEST_SOURCE_TYPE)
    ...
    if directory_name:
        result["directory_name"] = str(directory_name)
    if original_name:
        result["original_name"] = str(original_name)
    if page_index is not None:
        result["page_index"] = int(page_index)
